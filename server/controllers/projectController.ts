...
export const createProject = async (req: Request, res: Response) => {
  ...
  if (images.lenght < 2 || !productName) {
...
export const createVideo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!project || project.isGenerating) {
      return res.status(404).json({ message: "Genaration in progress" });
    }
    ...
    if (!project.generatedImage) {
      throw new Error("Genetaed image not found");
    }
    ...
    await prisma.project.update({
      where: { id: projectId },
      data: {
        generatedVideo: uploadResut.secure_url,
        isGenerating: false,
      },
    });
    ...
    res.json({
      ...
      videoUrl: uploadResut.secure_url,
    });
  } catch (error: any) {
    //update project status and error message
    await prisma.project.update({
...
export const getAllPublidhedProjects = async (req: Request, res: Response) => {
  try {
